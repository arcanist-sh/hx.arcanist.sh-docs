...
> **Measured with hx 0.8.0.** hx is faster than cabal on all four operations
> measured here — cold builds, CLI startup, no-op incremental rebuilds, and
> clean. stack was not re-measured for this release, so it is omitted rather than
> estimated.
...
- **No-op incremental rebuilds (≈5.7×)** short-circuit before any subprocess spawns — this path used to be *slower* than cabal until it was fixed.
